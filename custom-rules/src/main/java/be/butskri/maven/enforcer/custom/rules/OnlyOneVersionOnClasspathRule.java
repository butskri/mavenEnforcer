...
public class OnlyOneVersionOnClasspathRule implements EnforcerRule {
			...
			throws EnforcerRuleException
	{
		...
		try
		{
			...
			node.accept(visitor);
			List<List<DependencyNode>> conflictedVersionNumbers = filterConflictedVersions(visitor.getConflictedVersionNumbers());
	...
	private List<List<DependencyNode>> filterConflictedVersions(List<List<DependencyNode>> conflictedVersionNumbers) {
		List<List<DependencyNode>> result = new ArrayList<List<DependencyNode>>();
		for (List<DependencyNode> dependencyNodes : conflictedVersionNumbers) {
			if (artifactNameFilter().allows(fullArtifactNameFor(dependencyNodes))) {
				result.add(dependencyNodes);
			}
		}
		return result;
	}
	...
	private ArtifactNameFilter artifactNameFilter() {
		if (artifactNameFilter == null) {
			this.artifactNameFilter = new ArtifactNameFilter(this.dependenciesToBeChecked);
		}
		return artifactNameFilter;
	}

	private String fullArtifactNameFor(List<DependencyNode> dependencyNodes) {
		return getFullArtifactName(dependencyNodes.get(0).getArtifact());
	}
	 ...
	 * cached results are stored based on their id. The easiest way to do this
	 * is to return a hash computed from the values of your parameters. If your
	 * rule is not cacheable, then the result here is not important, you may
	 * return anything.
	 */
